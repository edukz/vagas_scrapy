...
class CompressedCache(IntelligentCache):
    ...
    def __init__(self, cache_dir: str = "data/cache", max_age_hours: int = 6, compression_level: int = 6):
        ...
        # Verificar se índice precisa ser reconstruído
    ...
    def _check_and_rebuild_index(self) -> None:
        """
        Verifica se o índice precisa ser reconstruído e faz isso automaticamente
        """
        try:
            # Contar arquivos de cache existentes
            cache_files = list(Path(self.cache_dir).glob("*.json.gz"))
            
            # Se há arquivos de cache mas o índice está vazio, reconstruir
            if len(cache_files) > 0 and len(self.index.entries) == 0:
                print(f"🔄 Detectados {len(cache_files)} arquivos de cache sem índice. Reconstruindo...")
                count = self.index.rebuild_index(self.cache_dir)
                print(f"✅ Índice reconstruído automaticamente: {count} arquivos indexados")
            elif len(self.index.entries) > 0:
                print(f"✅ Índice do cache carregado: {len(self.index.entries)} entradas")
        except Exception as e:
